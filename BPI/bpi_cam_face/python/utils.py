    ...
    def non_max_suppression_face(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, labels=()):
        ...
        for xi, x in enumerate(prediction):  # image index, image inference
            ...
            if labels and len(labels[xi]):
                l = labels[xi]
                v = np.zeros((len(l), nc + 5), dtype=x.dtype)  # 修复：使用dtype而不是device
                v[:, :4] = l[:, 1:5]  # box
                v[:, 4] = 1.0  # conf
                v[range(len(l)), l[:, 0].astype(int) + 5] = 1.0  # 修复：使用astype(int)而不是long()
            ...
            box = xywh2xyxy(x[:, :4])
            conf = x[:, 5:].max(axis=1).reshape(-1, 1)  # 修复：添加axis参数
