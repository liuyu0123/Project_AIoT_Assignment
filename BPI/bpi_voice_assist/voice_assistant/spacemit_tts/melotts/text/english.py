...
def get_g2p():
    ...
    if _g2p is None:
        try:
            ...
            print(f"Warning: G2p initialization failed: {e}")
            print("Will only use built-in CMU dictionary (cmudict.rep)")
            # 返回一个简单的备用函数，对于未知词返回空列表
            _g2p = lambda x: []
    return _g2p
...
def g2p_old(text):
    ...
    for w in words:
        if w.upper() in eng_dict:
            ...
            # 使用延迟初始化的 G2p
            g2p_instance = get_g2p()
            phone_list = list(filter(lambda p: p != " ", g2p_instance(w)))
