...
def generate_launch_description():
    ...
    return launch.LaunchDescription([
        ...
        # 控制启动顺序，方法2：使用 RegisterEventHandler
        # listener在talker启动后自动启动
        launch.actions.RegisterEventHandler(
            launch.event_handlers.OnProcessStart(
                target_action=action_node_talker,  # 监听talker的启动事件
                on_start=[action_node_listener],    # talker启动后触发listener
            )
        ),
    ])