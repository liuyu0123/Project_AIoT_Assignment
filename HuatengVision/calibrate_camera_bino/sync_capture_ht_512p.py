...
RESOLUTION = 12  # 5=1024x768 11=512×384（Windows是12）
SHOW_TIMES = 3   # 相机可视化放大倍数
...
RES_W, RES_H = 512, 384    # 目标分辨率（索引 1 通常是 720p）
...
def sdk_open_calib_camera(dev_idx):
    ...
    target_idx = RESOLUTION  # 或打印后看到的正确索引
...
def main():
    ...
    while True:
        ...
        cv2.imshow("calib_preview (ESC--quit, Space--capture)", cv2.resize(preview, (int(RES_W * SHOW_TIMES) , int(RES_H // 2 * SHOW_TIMES))))
