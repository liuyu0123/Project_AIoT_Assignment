...
def sync_record(left_idx, right_idx):
    ...
    while True:
        ...
        # cv2.imshow("stereo (ESC--Quit, Space--Record/Pause)", cv2.resize(preview, (0, 0), fx=0.25, fy=0.25))
        cv2.imshow("stereo (ESC--Quit, Space--Record/Pause)", cv2.resize(preview, (0, 0), fx=0.75, fy=0.75))
